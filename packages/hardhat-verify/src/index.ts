...
${contractURL}`);
      ...
      // First, try to verify the contract using the minimal input
      const { success: minimalInputVerificationSuccess, message: verificationMessage, }: VerificationResponse =
        await run(TASK_VERIFY_SOURCIFY_ATTEMPT_VERIFICATION, {
          address,
...
subtask(TASK_VERIFY_SOURCIFY_ATTEMPT_VERIFICATION)
  ...
  .setAction(
    ...
    { artifacts }): Promise<VerificationResponse> => {
      ...
      const chosenContract = Object.keys(buildInfo.output.contracts).findIndex(source=>source === artifact.sourceName)
